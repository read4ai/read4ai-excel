...
class PipelineIntegrationTest : FunSpec({
    ...
    test("full custom pipeline: all new implementations together") {
        val bytes = createMultiRegionXlsx()
        val pipeline = PipelineConfig(
            segmenter = ThreeLevelSegmenter(),
            headerDetector = MergeAwareHeaderDetector(),
    ...
    test("default pipeline still produces same results as before") {
        val bytes = createMultiRegionXlsx()
        val defaultDoc = ExcelParser.parse(bytes)
        val explicitDefaultDoc = ExcelParser.parse(bytes, pipeline = PipelineConfig())
